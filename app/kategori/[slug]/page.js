...
export async function generateMetadata({ params }) {
  ...
  if (!cat) return { title: "Kategori - Konsulentbud" };
  return {
    title: `${cat.name} - find hjælpere på Konsulentbud`,
    ...
    alternates: { canonical: `https://konsulentbud.dk/kategori/${cat.slug}` },
  };
}
...
export default async function CategoryPage({ params }) {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: cat.name, item: `https://konsulentbud.dk/kategori/${cat.slug}` },
    ],
  };
