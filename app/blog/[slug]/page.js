...
export async function generateMetadata({ params }) {
  ...
  if (!post) return { title: "Blogindlæg - Konsulentbud" };
  return {
    title: `${post.title} - Konsulentbud`,
    description: post.description,
    alternates: { canonical: `https://konsulentbud.dk/blog/${post.slug}` },
    openGraph: {
      title: post.title,
      description: post.description,
      type: "article",
...
export default function BlogPostPage({ params }) {
  ...
  const articleSchema = {
    ...
    author: { "@type": "Organization", name: "Konsulentbud" },
    publisher: { "@type": "Organization", name: "Konsulentbud" },
  };
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Blog", item: "https://konsulentbud.dk/blog" },
      { "@type": "ListItem", position: 3, name: post.title, item: `https://konsulentbud.dk/blog/${post.slug}` },
    ],
  };
  ...
  return (
    <div style={{ marginTop: 24, marginBottom: 60, maxWidth: 680 }}>
      ...
      <div style={{ fontSize: 12.5, color: "#5B6478", marginBottom: 16 }}>
        <Link href="/">Konsulentbud</Link> <ChevronRight size={11} style={{ display: "inline", verticalAlign: "middle" }} />{" "}
