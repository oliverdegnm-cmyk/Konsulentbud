...
En rigtig, delt budplatform for konsulentopgaver — bygget med Next.js og Postgres. Ud over selve bud-flowet (opret opgave, få bud) har konsulenter også en offentlig, søgbar profil under "Konsulenter", hvor de markerer hvilke kategorier de tager opgaver inden for og automatisk får besked, når en ny opgave matcher.
...
1. Opret et gratis GitHub-repo (f.eks. `konsulentbud`).
...
2. Klik "Add New Project" og vælg dit `konsulentbud`-repo.
3. Under "Environment Variables", tilføj:
   - **Name**: `DATABASE_URL`
   - **Value**: connection string'en fra Neon (trin 1)
4. Klik "Deploy". Efter et par minutter får du et live link som `konsulentbud.vercel.app`.

### 4. Det er det

Appen opretter selv sine databasetabeller (`tasks` og `bids`), første gang nogen besøger sitet eller kalder et API-endepunkt. Der er ikke flere manuelle trin — åbn bare dit `konsulentbud.vercel.app`-link.
...
Konsulentbud bruger rigtige konti med email og adgangskode. For at det virker, skal du tilføje en hemmelig nøgle, appen bruger til at signere login-sessioner:
...
Køb et domæne (f.eks. konsulentbud.dk) hos simply.com eller one.com, og tilføj det under dit projekts "Domains" i Vercel. Vercel guider dig igennem DNS-opsætningen.

## Sådan forbinder du email-bekræftelse (Resend)

Konsulentbud sender en bekræftelsesmail, når nogen opretter en konto. Det kræver Resend, en mailudbyder med en gratis kvote (100 mails/dag).
...
Til at starte med sender Resend automatisk fra `onboarding@resend.dev` — det virker med det samme, uden yderligere opsætning, men mails kan ende i spam hos nogle modtagere. For et mere troværdigt afsenderdomæne (f.eks. `noreply@konsulentbud.dk`), skal I under "Domains" hos Resend verificere jeres eget domæne (tilføje et par DNS-poster), og derefter sætte `RESEND_FROM_EMAIL` i Vercel til den nye adresse.

## Sådan forbinder du vedhæftede filer (Vercel Blob)

Konsulentbud bruger Vercel Blob til at gemme filer, folk vedhæfter til opgaver og beskeder. Det er markant nemmere end de andre integrationer, fordi det foregår direkte i Vercel:

1. Gå til dit projekt på vercel.com → fanen **"Storage"**
2. Klik **"Create Database"** → vælg **"Blob"**
3. Giv den et navn (f.eks. "konsulentbud-filer") og klik **"Create"**
...
Konsulentbud viser opgavernes placering på et Google Maps-kort med klynge-tal, ligesom Handyhand. Det kræver en Google Cloud-konto med faktureringsoplysninger tilknyttet (også selvom I bliver inden for den gratis kvote).

### 1. Opret et Google Cloud-projekt

Gå til [console.cloud.google.com](https://console.cloud.google.com), opret et nyt projekt (f.eks. "Konsulentbud"), og tilknyt en faktureringskonto under "Billing" — det er obligatorisk, men I betaler kun, hvis I overstiger de gratis 10.000 kald om måneden pr. tjeneste.
...
- **Browser-nøgle**: begræns den under "Application restrictions" til "Websites", og tilføj jeres Vercel-domæne (f.eks. `konsulentbud-xxxx.vercel.app/*` og evt. jeres eget domæne). Begræns den under "API restrictions" til kun **Maps JavaScript API**.
...
Konsulentbud bruger Stripe Connect til at holde betalingen, når et bud vælges, og frigive den til hjælperen (minus servicegebyr) når opgaven markeres som udført — præcis som Handyhands "HandyhandPay".
...
Konsulentbud skal vide, når en betaling er gennemført. Gå til **Developers → Webhooks → Add endpoint**:
...
Konsulentbud har en admin-side (`/admin`), hvor kundeservice kan se alle opgaver og brugere og om nødvendigt slette en opgave permanent. Der er ingen knap til at blive admin i selve appen — det gøres én gang direkte i databasen, så det ikke kan misbruges:
...
2. Kør denne kommando, med din egen email indsat:
   ...
   ```
3. Log ud og ind igen på Konsulentbud — der dukker nu et "Admin"-punkt op i menuen
