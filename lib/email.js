...
export async function sendVerificationEmail(toEmail, toName, token, origin) {
  ...
  try {
    await client.emails.send({
      from: process.env.RESEND_FROM_EMAIL || "Konsulentbud <onboarding@resend.dev>",
      to: toEmail,
      subject: "Bekræft din email hos Konsulentbud",
      html: `
        <div style="font-family:sans-serif; max-width:480px; margin:0 auto;">
          <h2 style="color:#241C35;">Hej ${toName}!</h2>
          <p style="color:#5B6478; line-height:1.6;">Klik på knappen nedenfor for at bekræfte din email og aktivere din konto på Konsulentbud.</p>
    ...
    await client.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif; max-width:480px; margin:0 auto;">
          ...
          <a href="${origin}" style="display:inline-block; background:#7C3AED; color:#fff; padding:12px 22px; border-radius:10px; text-decoration:none; font-weight:700; margin:16px 0;">Åbn Konsulentbud</a>
...
export async function sendContactEmail(toEmail, fromName, fromEmail, message) {
  ...
  try {
    await client.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif; max-width:480px; margin:0 auto;">
          <h2 style="color:#241C35;">Ny besked fra kontaktformularen</h2>
...
export async function sendPasswordResetEmail(toEmail, toName, token, origin) {
  ...
  try {
    await client.emails.send({
      from: process.env.RESEND_FROM_EMAIL || "Konsulentbud <onboarding@resend.dev>",
      to: toEmail,
      subject: "Nulstil din adgangskode hos Konsulentbud",
      html: `
        <div style="font-family:sans-serif; max-width:480px; margin:0 auto;">
          <h2 style="color:#241C35;">Hej ${toName}!</h2>
